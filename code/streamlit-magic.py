...
if st.button('make a card'):
    st.image(card_image(color = ccolor, card_name= cname, cardtext=new_lines(generate_text('When [CARDNAME] enters the battlefield')),subtype= csubtype, type=ctype, power=cpower, toughness= ctoughness, iscreature= ccreature))